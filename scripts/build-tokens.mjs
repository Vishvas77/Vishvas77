...
function cmdValidate() {
  ...
  const hpRel = "assets/hero/hero-poster.png";
  const hpAbs = path.join(ROOT, hpRel);
  const hpInfo = { exists: fs.existsSync(hpAbs) };
  if (hpInfo.exists) {
    const buf = fs.readFileSync(hpAbs);
    hpInfo.bytes = buf.length;
    const dims = pngDims(buf);
    hpInfo.width = dims ? dims.width : null;
    hpInfo.height = dims ? dims.height : null;
    if (!dims) fail("png", `${hpRel} is not a readable PNG`);
    else if (Math.abs(dims.width / dims.height - 16 / 9) > 0.02) fail("png", `${hpRel} is not 16:9 (${dims.width}x${dims.height})`);
    if (dims && dims.width < 1280) fail("png", `${hpRel} width ${dims.width} < 1280`);
    if (buf.length > 20 * 1024 * 1024) fail("png", `${hpRel} ${buf.length}B exceeds 20MB cap`);
  } else {
    fail("png", `${hpRel} missing — add the approved poster`);
  }
  report.suites.heroPoster = hpInfo;
  ...
  if (fs.existsSync(readmeAbs)) {
    ...
    if (!refs.includes("assets/hero/hero-poster.png")) fail("readme", "hero-poster.png is not embedded in README");
    for (const bad of ["assets/hero/hero-animated.svg", "assets/hero/hero-static.svg", "assets/hero/hero-static.png"]) {
      if (refs.includes(bad)) fail("readme", `forbidden README reference to legacy asset: ${bad}`);
    }
